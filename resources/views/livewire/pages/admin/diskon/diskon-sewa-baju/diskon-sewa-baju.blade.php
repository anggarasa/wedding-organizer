    ...
    <main class="p-4 lg:p-8">
        ...
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-900 mb-2">
                Kelola Diskon Sewa Baju
            </h1>
            <p class="text-gray-600">
                Buat dan kelola diskon sewa baju untuk pelanggan Anda
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-gradient-to-r from-violet-500 to-violet-600 rounded-lg shadow-lg p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-white text-2xl font-bold">{{ $diskonSewaBajus->where('status', 'aktif')->count()
                            }}
                        </h3>
                    </div>
            ...
            <div class="bg-gradient-to-r from-pink-500 to-pink-600 rounded-lg shadow-lg p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-white text-2xl font-bold">{{ $diskonSewaBajus->where('status',
            ...
            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg shadow-lg p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-white text-sm">Diskon Baju Kadaluarsa</p>
                        <h3 class="text-white text-2xl font-bold">{{ $diskonSewaBajus->where('status',
                            'kadaluarsa')->count() }}</h3>
                    </div>
        ...
        <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <livewire:pages.admin.diskon.diskon-sewa-baju.modal-diskon-sewa-baju />
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                @foreach ($diskonSewaBajus as $diskon)
                <tr>
                    ...
                    <td class="h-px w-52 whitespace-nowrap" x-data="{ modalId: null }">
                        <a class="block relative z-10 cursor-pointer"
                            @click="modalId = 'diskon-baju-detail_{{ $diskon->id }}'">
                            <div class="px-6 py-2 flex -space-x-2">
                                @php
                                $displayedBajus = $diskon->sewaBajus->take(3);
                                $remainingBajus = $diskon->sewaBajus->count() - 3;
                                @endphp

                                @foreach ($displayedBajus as $baju)
                                <div class="inline-flex">
                                    ...
                                    <img class="inline-block size-6 rounded-full object-cover ring-2 ring-white dark:ring-neutral-900"
                                        src="{{ asset('storage/sewa-baju/'. $image->image) }}" alt="{{ $baju->name }}">
                                    @endforeach
                                    <span
                                ...
                                <div class="inline-flex">
                                    <span
                                        ...
                                        <span class="font-medium leading-none">{{ $remainingBajus
                                            }}+</span>
                                    </span>
                                    <span
                                        class="opacity-0 transition-opacity block absolute invisible z-10 py-1 px-2 bg-gray-900 text-xs font-medium text-white rounded shadow-sm dark:bg-neutral-700">
                                        {{ $remainingBajus }} more paket(s)
                                    </span>
                                </div>
                                @endif
                            </div>
                        ...
                        <!-- Modal -->
                        <div x-show="modalId === 'diskon-baju-detail_{{ $diskon->id }}'" class="fixed
                        inset-0 z-50 p-4 bg-black bg-opacity-50 flex items-center justify-center"
                            ...
                            <div class="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6">
                                <h2 class="text-xl font-bold mb-4">Sewa Baju Terkait</h2>
                    ...
                    <td class="size-px whitespace-nowrap">
                        <div class="px-6 py-1.5 space-x-4">
                            ...
                            <button type="button"
                                @click="showDeleteModal = 'modal-delete-diskon-baju_{{ $diskon->id }}'"
                    ...
                    <div class="relative flex items-center justify-center min-h-screen p-4">
                        <div class="relative w-full max-w-md p-6 bg-white rounded-xl shadow-lg">
                            ...
                            <div class="flex justify-center space-x-3">
                                ...
                                <button type="button" wire:click="deleteDiskonSewaBaju({{ $diskon->id }})"
        ...
        <!-- Pagination -->
        {{-- {{ $diskonSewaBajus->links('vendor.pagination.tailwind') }} --}}
        <!-- Pagination -->
