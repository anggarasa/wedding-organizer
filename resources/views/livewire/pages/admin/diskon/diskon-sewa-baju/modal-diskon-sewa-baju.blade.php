<div x-data="{ showModal: false }">
    <button @click="showModal = true"
        ...
        Tambah Diskon Sewa Baju
    </button>

    <div x-show="showModal" @modal-diskon-baju.window="showModal = true"
        @close-modal-diskon-baju.window="showModal = false" class="fixed inset-0 z-30 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center">
            ...
            <div
                ...
                <form wire:submit="{{ $isEdit == true ? 'updateDiskonSewaBaju' : 'createDiskonSewaBaju' }}"
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Pilih Sewa Baju</label>

                        <!-- Selected Paket Display -->
                        <div class="mb-4 flex flex-wrap gap-2">
                            @foreach($selectBajuDetail as $baju)
                            <div
                                class="inline-flex items-center bg-violet-100 text-violet-800 px-3 py-1 rounded-full text-sm">
                                <span>{{ $baju->name }}</span>
                                <button type="button" wire:click="toggleKebaya({{ $baju->id }})"
                        ...
                        <button type="button" wire:click="$toggle('dropdownSelect')"
                            ...
                            <span>
                                {{ count($selectBajus) ? count($selectBajus) . ' baju dipilih' : 'Pilih
                                Baju' }}
                            </span>
                                    ...
                                    <div wire:click="toggleKebaya({{ $baju->id }})"
                                        ...
                                        <div class="relative">
                                            ...
                                            <img src="{{ asset('imgs/logo/logo-aplikasi-ym.svg') }}"
                                                class="w-16 h-16 rounded-lg object-cover" />
                                            @endforelse
                                            @if(in_array($baju->id, $selectBajus))
                                            <div
